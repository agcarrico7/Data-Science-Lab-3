...
```{r 4}
# use scatterplot to visualize k-means clusters
ggDatakmean <- cbind(iris, cluster_assignments)
ggDatakmean$cluster <- as.factor(ggDatakmean$cluster_assignments)
print(ggDatakmean)
ggplot(ggDatakmean, aes(x = Sepal.Length, 
                                  y = Petal.Length, color=cluster)) + 
  labs(x = "Sepal Length", y = "Petal Length", title = "K-means clustering visualized for Petal Length and Sepal Length") +
  geom_point(size=2)

# use scatterplot to visualize complete correlation Hierarchical clusters
ggDatacomcor <- cbind(iris, cor_comp_3)
ggDatacomcor$cluster <- as.factor(ggDatacomcor$cor_comp_3)
ggplot(ggDatacomcor, aes(x = Sepal.Length, 
                                  y = Petal.Length, color=cluster)) + 
  labs(x = "Sepal Length", y = "Petal Length", title = "Complete Correlation Hierarchical clustering visualized for Petal Length and Sepal Length") +
  geom_point(size=2)

# use scatterplot to visualize average correlation clusters
ggDatacoravg <- cbind(iris, cor_avg_3)
ggDatacoravg$cluster <- as.factor(ggDatacoravg$cor_avg_3)
ggplot(ggDatacoravg, aes(x = Sepal.Length, 
                                  y = Petal.Length, color=cluster)) + 
  labs(x = "Sepal Length", y = "Petal Length", title = "Average Correlation Hierarchical clustering visualized for Petal Length and Sepal Length") +
  geom_point(size=2)
```
The k-means method to cluster in 3 groups with 50 iterations contained 69 observations in one group, 56 in another, and 25 in the last. These groups are better than any of the Hierarchical methods which makes sense given that k-means clustering is more effective on large data sets.